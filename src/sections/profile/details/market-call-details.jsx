...
export default function ProfileMarketCallDetails() {
  const { id } = useParams();
  ...
  const { data: marketCall = {}, isLoading: marketCallLoading } = useQuery({
    ...
    queryFn: async () => {
      const { data } = await ProfileMarketcallService.get({
        _id: id,
  ...
  const { mutate: subscribeMarketcall, isPending } = useMutation({
    mutationFn: () => {
      ...
      console.log('clients', id, { clients: [...clients, userId] });
  ...
  const renderDescription = <Typography>{marketCall?.description}</Typography>;
