...
import { Card, CardContent, Typography, Box, Avatar, Stack, Button } from '@mui/material';
...
export default function ProfileMarketCallCard({ marketCall, buttonText }) {
  const { createdBy, portfolio, _id: id, title, description } = marketCall;
  const router = useRouter();

  const renderCreatedBy = (
    <Typography>by {`${createdBy.firstName} ${createdBy.lastName}`}</Typography>
  );
  const renderTitle = (
  ...
  const renderButton = (
    <Button
      ...
      color="inherit"
      sx={{ mt: 1, width: '100%' }}
      onClick={() => router.push(`/profile/market-call/details/${id}`)}
    >
      {buttonText}
    </Button>
  );
...
ProfileMarketCallCard.propTypes = {
